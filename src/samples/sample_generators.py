...
def generate_random_values_two_level_model(model, resource_limit, major_cost_limit, restriction_matrices_limit, minor_cost_limits, is_int_type = False):
    ...
    model.resource_limit(get_random([0, resource_limit]))

    model.major_cost_coefs(
        [get_random([0, major_cost_limit]) for i in range(len(model.major_cost_coefs))]
    )
    ...
    for i in range(len(minors)):
        for j in range(len(minors[i])):
            minors[i][j] = get_random([0, minor_cost_limits])
